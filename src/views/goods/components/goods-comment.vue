...
export default {
  ...
  setup() {
    ...
    findGoodsCommentInfo({ id: goods.value.id }).then(data => {
      // 设置数据之前，tags数组前去追加 有图tag 全部评价tag
    ...
    // 初始化需要发请求，并且筛选条件发生变化也要发请求
    const commentList = ref([])
    const total = ref(0)
    watch(reqParams, () => {
      findGoodsCommentList({ id: goods.value.id, params: reqParams }).then(data => {
