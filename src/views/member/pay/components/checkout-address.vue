<template>
  <div class="checkout-address">
    <div class="text">
      ...
      <a href="javascript:;" @click="addDialog(showAddress)">修改地址</a>
    </div>
    <div class="action">
      <XtxButton @click="openDialog" class="btn">切换地址</XtxButton>
      <XtxButton @click="addDialog({})" class="btn">添加地址</XtxButton>
    </div>
  </div>
  <!-- 对话框组件 -->
  ...
  <!-- 收货地址添加编辑组件 -->
  <AddressEdit @on-success="successHandler" ref="address"></AddressEdit>
</template>
<script>
import { ref } from 'vue'
...
export default {
  ...
  // 2.如果你的组件是渲染的代码片段，vue3规范，需要在emits中声明你所要触发的自定义事件
  // 3.vue3提倡：你触发了自定义事件，需要在emits选项声明一下
  emits: ['change'],
  setup(props, { emit }) {
    // 1.找到默认收货地址
    ...
    const addDialog = (addresses) => {
      // 添加的时候传入空对象，修改的时候传入有数据的对象
      address.value.open(addresses)
    }
    const successHandler = (formData) => {
      // 根据formData中的id，取当前地址列表中查找，有就是修改，没有就是添加
      ...
      if (address) {
        ...
        // 如果是添加，往list中追加一条
        // 当在修改formData的时候数组中的数据也会发生改变,因为是同一个引用地址
        // 当重新对话框需要清空之前的信息
        // 克隆formData数据
    ...
    return { showAddress, visibleDialog, selectedAddress, confirmAddressFn, openDialog, addDialog, address, successHandler }
  }
}
</script>
